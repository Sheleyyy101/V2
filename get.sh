...
rm -rf proxynofilter.txt proxys.txt proxy.txt 2>/dev/null
echo "Downloading proxy."
wget "https://www.proxy-list.download/api/v1/get?type=http&anon=anonymous" -O proxynofilter.txt 2>/dev/null
...
cat proxy.txt >> proxynofilter.txt 2>/dev/null
rm -rf proxy.txt
wget "https://www.proxy-list.download/api/v1/get?type=http&anon=elite" -O proxy.txt 2>/dev/null
cat proxy.txt >> proxynofilter.txt 2>/dev/null
rm -rf proxy.txt
#python3 proxyScraper.py -p http 2>/dev/null
#cat output.txt >> proxynofilter.txt
#rm -rf output.txt
wget "http://ipstresser.pro/prox13/proxies.txt" -O proxy.txt 2>/dev/null
cat proxy.txt >> proxynofilter.txt 2>/dev/null
rm -rf proxy.txt
#wget "http://proxydb.net/?protocol=http" -O proxy.txt 2>/dev/null
...
cat proxy.txt >> proxynofilter.txt
rm -rf proxy.txt
#wget "https://openproxy.space/list/http" -O proxy.txt 2>/dev/null
#cat proxy.txt >> proxynofilter.txt
#rm -rf proxy.txt
#wget "https://www.proxy-list.download/HTTP" -O proxy.txt 2>/dev/null
#cat proxy.txt >> proxynofilter.txt
#rm -rf proxy.txt
#wget "https://api.proxyscrape.com/v2/account/datacenter_shared/proxy-list?auth=8fo1gdqz21oamvso6zdy&type=getproxies&country[]=all&protocol=http&format=normal&status=all" -O proxy.txt 2>/dev/null
#cat proxy.txt >> proxynofilter.txt
#rm -rf proxy.txt
